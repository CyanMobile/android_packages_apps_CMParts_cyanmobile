...
public class NBColorPickerDialog extends Dialog {
    private static int CENTER_X = 100;
    private static int CENTER_Y = 100;
    private static int CENTER_RADIUS = 32;

    public interface OnColorChangedListener {
        void NBcolorChanged(int NBcolor);
	/** @hide */
        void NBcolorUpdate(int NBcolor);
    }

    private OnColorChangedListener mListener;
    private int mInitialColor;

    private class NBColorPickerView extends View {
        private Paint mPaint;
        private Paint mCenterPaint;
        private final int[] mColors;
        private OnColorChangedListener mListener;
        
        NBColorPickerView(Context c, OnColorChangedListener l, int NBcolor) {
            super(c);
            mListener = l;
            mColors = new int[] {
        ...
        public void setCenterColor(int NBcolor) {
            mCenterPaint.setColor(NBcolor);
            invalidate();
        }
        
        public void setTransparency(int alpha) {
            int NBcolor = mCenterPaint.getColor();            
            int newColor = Color.argb(alpha, Color.red(NBcolor), Color.green(NBcolor), Color.blue(NBcolor));
        ...
        private int rotateColor(int NBcolor, float rad) {
            float deg = rad * 180 / 3.1415927f;
            int r = Color.red(NBcolor);
            int g = Color.green(NBcolor);
            int b = Color.blue(NBcolor);
            ...
            return Color.argb(Color.alpha(NBcolor), pinToByte(ir),
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (mTrackingCenter) {
                        ...
                        int NBcolor = interpColor(mColors, unit);
                        mCenterPaint.setColor(NBcolor);
                        mEditText.setText(convertToARGB(NBcolor));
                        mListener.NBcolorUpdate(NBcolor);
			invalidate();
                    }
                    break;
                case MotionEvent.ACTION_UP:
                    if (mTrackingCenter) {
                        if (inCenter) {
                            mListener.NBcolorChanged(mCenterPaint.getColor());
                        }
    ...
    private String convertToARGB(int NBcolor) {
        String alpha = Integer.toHexString(Color.alpha(NBcolor));
        String red = Integer.toHexString(Color.red(NBcolor));
        String green = Integer.toHexString(Color.green(NBcolor));
        String blue = Integer.toHexString(Color.blue(NBcolor));        
    ...
    private NBColorPickerView mColorPickerView;
    private SeekBar mTransparencyBar;

    public NBColorPickerDialog(Context context, OnColorChangedListener listener, int initialColor) {
        super(context);
        mContext = context;
        mListener = listener;
        mInitialColor = initialColor;
        CENTER_Y=CENTER_X=context.getResources().getDimensionPixelSize(com.cyanogenmod.cmparts.R.dimen.color_picker_center);
        CENTER_RADIUS=context.getResources().getDimensionPixelSize(com.cyanogenmod.cmparts.R.dimen.color_picker_radius);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv.setText(com.cyanogenmod.cmparts.R.string.msg_color_picker);
        layout.addView(tv, layoutParams);
        
        mColorPickerView = new NBColorPickerView(getContext(), OnColorChangedListener, mInitialColor);        
        layout.addView(mColorPickerView, layoutParams);
        
        mTransparencyBar = new SeekBar(mContext);
        mTransparencyBar.setMax(255);
		mTransparencyBar.setProgressDrawable(new TextSeekBarDrawable(mContext.getResources(),
                         mContext.getString(com.cyanogenmod.cmparts.R.string.pref_dialog_color_picker_alpha), true));
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            try {
                ...
                if (s2.length() == 6 || s2.length() == 8) {
                    int NBcolor = convertToColorInt(s2);
                    mColorPickerView.setCenterColor(NBcolor);
                    mTransparencyBar.setProgress(Color.alpha(NBcolor));
                }            
            }
