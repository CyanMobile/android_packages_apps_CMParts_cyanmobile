...
public class SBColorPickerDialog extends Dialog {
    private static int CENTER_X = 100;
    private static int CENTER_Y = 100;
    private static int CENTER_RADIUS = 32;

    public interface OnColorChangedListener {
        void SBcolorChanged(int SBcolor);
	/** @hide */
        void SBcolorUpdate(int SBcolor);
    }

    private OnColorChangedListener mListener;
    private int mInitialColor;

    private class SBColorPickerView extends View {
        private Paint mPaint;
        private Paint mCenterPaint;
        private final int[] mColors;
        private OnColorChangedListener mListener;
        
        SBColorPickerView(Context c, OnColorChangedListener l, int SBcolor) {
            super(c);
            mListener = l;
            mColors = new int[] {
        ...
        public void setCenterColor(int SBcolor) {
            mCenterPaint.setColor(SBcolor);
            invalidate();
        }
        
        public void setTransparency(int alpha) {
            int SBcolor = mCenterPaint.getColor();            
            int newColor = Color.argb(alpha, Color.red(SBcolor), Color.green(SBcolor), Color.blue(SBcolor));
        ...
        private int rotateColor(int SBcolor, float rad) {
            float deg = rad * 180 / 3.1415927f;
            int r = Color.red(SBcolor);
            int g = Color.green(SBcolor);
            int b = Color.blue(SBcolor);
            ...
            return Color.argb(Color.alpha(SBcolor), pinToByte(ir),
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (mTrackingCenter) {
                        ...
                        int SBcolor = interpColor(mColors, unit);
                        mCenterPaint.setColor(SBcolor);
                        mEditText.setText(convertToARGB(SBcolor));
                        mListener.SBcolorUpdate(SBcolor);
			invalidate();
                    }
                    break;
                case MotionEvent.ACTION_UP:
                    if (mTrackingCenter) {
                        if (inCenter) {
                            mListener.SBcolorChanged(mCenterPaint.getColor());
                        }
    ...
    private String convertToARGB(int SBcolor) {
        String alpha = Integer.toHexString(Color.alpha(SBcolor));
        String red = Integer.toHexString(Color.red(SBcolor));
        String green = Integer.toHexString(Color.green(SBcolor));
        String blue = Integer.toHexString(Color.blue(SBcolor));        
    ...
    private SBColorPickerView mColorPickerView;
    private SeekBar mTransparencyBar;

    public SBColorPickerDialog(Context context, OnColorChangedListener listener, int initialColor) {
        super(context);
        mContext = context;
        mListener = listener;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mColorPickerView = new SBColorPickerView(getContext(), OnColorChangedListener, mInitialColor);        
    ...
    private OnColorChangedListener OnColorChangedListener = new OnColorChangedListener() {
        public void SBcolorChanged(int SBcolor) {
           mListener.SBcolorChanged(SBcolor);
           dismiss();
        }
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            try {
                ...
                if (s2.length() == 6 || s2.length() == 8) {
                    int SBcolor = convertToColorInt(s2);
                    mColorPickerView.setCenterColor(SBcolor);
                    mTransparencyBar.setProgress(Color.alpha(SBcolor));
                }            
            }
