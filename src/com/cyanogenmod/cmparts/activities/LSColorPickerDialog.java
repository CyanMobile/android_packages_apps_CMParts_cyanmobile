...
public class LSColorPickerDialog extends Dialog {
    private static int CENTER_X = 100;
    private static int CENTER_Y = 100;
    private static int CENTER_RADIUS = 32;

    public interface OnColorChangedListener {
        void LScolorChanged(int LScolor);
	/** @hide */
        void LScolorUpdate(int LScolor);
    }

    private OnColorChangedListener mListener;
    private int mInitialColor;

    private class LSColorPickerView extends View {
        private Paint mPaint;
        private Paint mCenterPaint;
        private final int[] mColors;
        private OnColorChangedListener mListener;
        
        LSColorPickerView(Context c, OnColorChangedListener l, int LScolor) {
            super(c);
            mListener = l;
            mColors = new int[] {
        ...
        public void setCenterColor(int LScolor) {
            mCenterPaint.setColor(LScolor);
            invalidate();
        }
        
        public void setTransparency(int alpha) {
            int LScolor = mCenterPaint.getColor();            
            int newColor = Color.argb(alpha, Color.red(LScolor), Color.green(LScolor), Color.blue(LScolor));
        ...
        private int rotateColor(int LScolor, float rad) {
            float deg = rad * 180 / 3.1415927f;
            int r = Color.red(LScolor);
            int g = Color.green(LScolor);
            int b = Color.blue(LScolor);
            ...
            return Color.argb(Color.alpha(LScolor), pinToByte(ir),
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (mTrackingCenter) {
                        ...
                        int LScolor = interpColor(mColors, unit);
                        mCenterPaint.setColor(LScolor);
                        mEditText.setText(convertToARGB(LScolor));
                        mListener.LScolorUpdate(LScolor);
			invalidate();
                    }
                    break;
                case MotionEvent.ACTION_UP:
                    if (mTrackingCenter) {
                        if (inCenter) {
                            mListener.LScolorChanged(mCenterPaint.getColor());
                        }
    ...
    private String convertToARGB(int LScolor) {
        String alpha = Integer.toHexString(Color.alpha(LScolor));
        String red = Integer.toHexString(Color.red(LScolor));
        String green = Integer.toHexString(Color.green(LScolor));
        String blue = Integer.toHexString(Color.blue(LScolor));        
    ...
    private LSColorPickerView mColorPickerView;
    private SeekBar mTransparencyBar;

    public LSColorPickerDialog(Context context, OnColorChangedListener listener, int initialColor) {
        super(context);
        mContext = context;
        mListener = listener;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mColorPickerView = new LSColorPickerView(getContext(), OnColorChangedListener, mInitialColor);        
    ...
    private OnColorChangedListener OnColorChangedListener = new OnColorChangedListener() {
        public void LScolorChanged(int LScolor) {
           mListener.LScolorChanged(LScolor);
           dismiss();
        }
        public void LScolorUpdate(int LScolor) {
	  mListener.LScolorUpdate(LScolor);
        }
    };
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            try {
                ...
                if (s2.length() == 6 || s2.length() == 8) {
                    int LScolor = convertToColorInt(s2);
                    mColorPickerView.setCenterColor(LScolor);
                    mTransparencyBar.setProgress(Color.alpha(LScolor));
                }            
            }
