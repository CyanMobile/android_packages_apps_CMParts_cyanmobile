...
public class CRColorPickerDialog extends Dialog {
    private static int CENTER_X = 100;
    private static int CENTER_Y = 100;
    private static int CENTER_RADIUS = 32;

    public interface OnColorChangedListener {
        void CRcolorChanged(int CRcolor);
	/** @hide */
        void CRcolorUpdate(int CRcolor);
    }

    private OnColorChangedListener mListener;
    private int mInitialColor;

    private class CRColorPickerView extends View {
        private Paint mPaint;
        private Paint mCenterPaint;
        private final int[] mColors;
        private OnColorChangedListener mListener;
        
        CRColorPickerView(Context c, OnColorChangedListener l, int CRcolor) {
            super(c);
            mListener = l;
            mColors = new int[] {
        ...
        public void setCenterColor(int CRcolor) {
            mCenterPaint.setColor(CRcolor);
            invalidate();
        }
        
        public void setTransparency(int alpha) {
            int CRcolor = mCenterPaint.getColor();            
            int newColor = Color.argb(alpha, Color.red(CRcolor), Color.green(CRcolor), Color.blue(CRcolor));
        ...
        private int rotateColor(int CRcolor, float rad) {
            float deg = rad * 180 / 3.1415927f;
            int r = Color.red(CRcolor);
            int g = Color.green(CRcolor);
            int b = Color.blue(CRcolor);
            ...
            return Color.argb(Color.alpha(CRcolor), pinToByte(ir),
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (mTrackingCenter) {
                        ...
                        int CRcolor = interpColor(mColors, unit);
                        mCenterPaint.setColor(CRcolor);
                        mEditText.setText(convertToARGB(CRcolor));
                        mListener.CRcolorUpdate(CRcolor);
			invalidate();
                    }
                    break;
                case MotionEvent.ACTION_UP:
                    if (mTrackingCenter) {
                        if (inCenter) {
                            mListener.CRcolorChanged(mCenterPaint.getColor());
                        }
    ...
    private String convertToARGB(int CRcolor) {
        String alpha = Integer.toHexString(Color.alpha(CRcolor));
        String red = Integer.toHexString(Color.red(CRcolor));
        String green = Integer.toHexString(Color.green(CRcolor));
        String blue = Integer.toHexString(Color.blue(CRcolor));        
    ...
    private CRColorPickerView mColorPickerView;
    private SeekBar mTransparencyBar;

    public CRColorPickerDialog(Context context, OnColorChangedListener listener, int initialColor) {
        super(context);
        mContext = context;
        mListener = listener;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mColorPickerView = new CRColorPickerView(getContext(), OnColorChangedListener, mInitialColor);        
    ...
    private OnColorChangedListener OnColorChangedListener = new OnColorChangedListener() {
        public void CRcolorChanged(int CRcolor) {
           mListener.CRcolorChanged(CRcolor);
           dismiss();
        }
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            try {
                ...
                if (s2.length() == 6 || s2.length() == 8) {
                    int CRcolor = convertToColorInt(s2);
                    mColorPickerView.setCenterColor(CRcolor);
                    mTransparencyBar.setProgress(Color.alpha(CRcolor));
                }            
            }
